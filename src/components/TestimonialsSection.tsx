...
const TestimonialCard: React.FC<{ testimonial: any }> = ({ testimonial }) => (
    <div className="bg-white dark:bg-brand-dark-3 p-8 rounded-2xl border border-gray-200 dark:border-gray-800/60 h-full flex flex-col justify-between shadow-lg z-0">
        <div>
...
const TestimonialsSection: React.FC = () => {
    ...
    return (
        <section id="testimonials" ref={sectionRef} className="min-h-screen flex items-center bg-brand-light-2 dark:bg-brand-dark-2 overflow-x-hidden">
            <div className="container mx-auto px-6 py-20 lg:py-0">
                ...
                {/* Carousel + Botões de Navegação */}
                <div className={`relative max-w-screen-xl mx-auto ${getTransitionClasses('', 'delay-200')}`}>
                    {/* Desktop carousel */}
                    <div className="hidden lg:block overflow-hidden">
                        ...
                        >
                            {[...testimonials, ...testimonials, ...testimonials].map((testimonial, index) => (
                                <div key={index} className="w-full lg:w-1/3 flex-shrink-0" style={{ flexBasis: `${100 / 3}%` }}>
                    ...
                    <button onClick={handlePrev} className="absolute top-1/2 -translate-y-1/2 -left-6 lg:-left-10 xl:-left-12 text-gray-400 dark:text-gray-500 hover:text-gray-300 dark:hover:text-white transition-colors p-2 rounded-full focus:outline-none z-10" aria-label="Anterior">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" />
                        </svg>
                    </button>
                    <button onClick={handleNext} className="absolute top-1/2 -translate-y-1/2 -right-6 lg:-right-10 xl:-right-12 text-gray-400 dark:text-gray-500 hover:text-gray-300 dark:hover:text-white transition-colors p-2 rounded-full focus:outline-none z-10" aria-label="Próximo">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
